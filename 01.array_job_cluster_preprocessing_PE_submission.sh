...
while getopts "c:h" opt; do
  case $opt in
    h)
      echo "This will run 01.array_job_cluster_preprocessing_PE_submission with the samples in doc/samples_id.txt.
            OPTIONS:
            -c: path to config file.
            Usage: bash ~/01.array_job_cluster_preprocessing_PE_submission.sh -c /path/to/config/file/file.config" 
      ;;
    c)
	config_file=$OPTARG
      ;;
...
qsub -P AG -A arubio -l h_vmem=5G -v config_file=$config_file -N "preprocessing" -t 1:${#samples_id[@]} -tc 5 -e ${analysisDir}/sge_log/\$TASK_ID.e.log -o ${analysisDir}/sge_log/\$TASK_ID.o.log ${srcDir}01.array_job_cluster_preprocessing_PE.sh
